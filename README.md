...
受保护 Skill：`stage` / `mapick` / `tasa` 不可删除。
...
安装 ──→ 首次使用 ──→ 持续使用 ──→ 频率下降 ──→ 僵尸状态 ──→ 卸载
  ↑           ↑             ↑              ↑             ↑          ↑
 Stage       Stage          Stage          Stage         Stage     用户
  扫描        记录           记录           记录          标记      触发
```
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v3.0 | 2026-04-18 | 新增 M3 套装推荐，改名为 Mapickii |
...
- `docs/dev_stage/M1_Skill生命周期管理.md` —— M1 设计文档
- `plan/mapickii/v3_dev_plan.md` —— v3 开发计划

## License

[MIT](LICENSE) &copy; 2026 Mapick.AI
